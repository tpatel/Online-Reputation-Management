...
To try to keep it simple we decided to estimate where on a scale between negative and positive the opinion of the people would be on a certain matter.
...
To perform this text-based classification we chose to use a Support Vector Machine (SVM) because that approach appeared to get the best results when comparing results from [references].
...
In order to do this we manually classified a number of tweets and then used the lexicon and the SVM to classify those same tweets as well and then we compared the results.
